...
def vgg_model(x_train):
  ...
  x = Dropout(0.4)(x)
  x = Flatten()(x)
  x = Dense(num_classes,kernel_initializer=initializers.glorot_uniform(seed=1234) ,activation="softmax")(x)
...
Y_pred = model.predict(images.astype("float32"))
a = np.array([ np.where(out==np.amax(out))[0][0] for out in Y_pred])

np.savetxt("predection.txt",a)
